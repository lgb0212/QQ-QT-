...
#define DATETIMECH      qPrintable(QDateTime::currentDateTime().toString("yyyy年MM月dd日 HH时mm分ss秒"))
...
public:
    ...
    static QString getFileVersion(QString appName);
    //检测第三方程序是否运行
    static bool CheckAppRunningStatus(const QString &appName);
    //强制退出第三方软件
    static void KillAppProcess(const QString &appName);
    //初始化随机数种子
    static void initRand();
    //生成6位数的随机数编码
    static QString generateUniqueRandomNumber();
    //设置系统音量
    static bool setVolumeLevel(int level);
    ...
    static quint16 getModbus16ccitt(quint8 *data, int len);
    ...
    //获取CSV数据
    static QList<QStringList> getCSVdData(const QString &filter);
    //写入CSV文件数据
    static void setCSVdData(const QString &filter,QList<QStringList> list);
    //修改CSV文件数据中的某一列
    static void setCSVdData(const QString &filter ,QStringList list , int col);
    //获取保存的文件
