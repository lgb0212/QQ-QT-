...
DbConnectThread::DbConnectThread(QObject *parent) : QThread(parent)
{

}

void DbConnectThread::run()
{
    for (;;)
    {
        bool isok = countSQLServer(IP,port,sqlname,sa,password,connName);
        emit Connection_click(isok);
        while (1)
        {
            emit Connection_click(isok);
            msleep(3000);
...
bool DbConnectThread::countSQLServer(const QString &IP,const int port,const QString &sqlname,const QString &sa,const QString &password,const QString &connName)
{
...
//                                );
    ...
    db.setConnectOptions("MYSQL_OPT_RECONNECT=1;"); //断开连接自动重连
    if(db.open()){
         emit Connection_click(true);
        return true;
    }
    else{
        return false;
...
void DbConnectThread::setConnName(const QString &IP,const int port,const QString &sqlname,const QString &sa,const QString &password,const QString &connName)
{
    this->IP = IP;
    this->port = port;
...
void DbCountThread::select()
{
    ...
    qDebug()<<"DbCountThread::Db count = "<<count;
...
void DbPage::bindData(const QString &sql)
{
    ...
    if (labPageCurrent != 0) {
        labPageCurrent->setText(QString("第 %1 条").arg(pageCurrent));
    }
