...
int my_QUIHelper::getScreenIndex()
{
    //需要对多个屏幕进行处理
    int screenIndex = 0;
...
QRect my_QUIHelper::getScreenRect(bool available)
{
    QRect rect;
    int screenIndex = my_QUIHelper::getScreenIndex();
    if (available) {
#if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
...
int my_QUIHelper::deskWidth()
{
    return getScreenRect().width();
}

int my_QUIHelper::deskHeight()
{
    return getScreenRect().height();
}

QWidget *my_QUIHelper::centerBaseForm = 0;
void my_QUIHelper::setFormInCenter(QWidget *form)
{
...
void my_QUIHelper::newDir(const QString &dirName)
{
    ...
    if (!strDir.startsWith("/") && !strDir.contains(":/")) {
        strDir = QString("%1/%2").arg(my_QUIHelper::appPath()).arg(strDir);
    }
...
void my_QUIHelper::sleep(int msec)
{
    if (msec <= 0) {
        return;
    }
...
void my_QUIHelper::setFont(int fontSize)
{
    QFont font;
...
void my_QUIHelper::setCode(bool utf8)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
...
void my_QUIHelper::setTranslator(const QString &qmFile)
{
    //过滤下不存在的就不用设置了
...
void my_QUIHelper::initAll()
{
    //初始化随机数种子
    my_QUIHelper::initRand();
    //设置样式风格
    my_QUIHelper::setStyle();
    //设置字体
    my_QUIHelper::setFont(13);
    //设置编码
    my_QUIHelper::setCode();
    //设置翻译文件支持多个
    my_QUIHelper::setTranslator(":/qm/widgets.qm");
    my_QUIHelper::setTranslator(":/qm/qt_zh_CN.qm");
    my_QUIHelper::setTranslator(":/qm/designer_zh_CN.qm");
}

void my_QUIHelper::setFramelessForm(QWidget *widgetMain, bool tool, bool top, bool menu)
{
...
int my_QUIHelper::showMessageBox(const QString &info, int type, int closeSec, bool exec)
{
    int result = 0;
    if (type == 0) {
...
void my_QUIHelper::showMessageBoxInfo(const QString &info, int closeSec, bool exec)
{
...
void my_QUIHelper::showMessageBoxError(const QString &info, int closeSec, bool exec)
{
...
int my_QUIHelper::showMessageBoxQuestion(const QString &info)
{
...
QString my_QUIHelper::getXorEncryptDecrypt(const QString &value, char key)
{
    //矫正范围外的数据
    if (key < 0 || key >= 127) {
        key = 127;
...
uchar my_QUIHelper::getOrCode(const QByteArray &data)
{
    int len = data.length();
    uchar result = 0;
    for (int i = 0; i < len; i++) {
...
uchar my_QUIHelper::getCheckCode(const QByteArray &data)
{
    int len = data.length();
    uchar temp = 0;
...
void my_QUIHelper::initTableView(QTableView *tableView, int rowHeight, bool headVisible, bool edit, bool stretchLast)
{
    //取消自动换行
...
void my_QUIHelper::openFile(const QString &fileName, const QString &msg)
{
#ifdef __arm__
    return;
#endif
    ...
    if (my_QUIHelper::showMessageBoxQuestion(msg + "成功!确定现在就打开吗?") == QMessageBox::Yes) {
        QString url = QString("file:///%1").arg(fileName);
        QDesktopServices::openUrl(QUrl(url, QUrl::TolerantMode));
    }
}

bool my_QUIHelper::checkIniFile(const QString &iniFile)
{
