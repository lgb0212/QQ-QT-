...
void QUIMessageBox::setMessage(const QString &msg, int type, int closeSec)
{
    ...
    if (type == 0) {
        if (exist) {
            this->labIcoMain->setStyleSheet("border-image: url(:/XYKJ.ico);");
        } else {
...
QString QUIHelper::getWMIC(const QString &cmd)
{
...
QString QUIHelper::getCpuName()
{
    return getWMIC("wmic cpu get name");
}
QString QUIHelper::getCpuId()
{
    return getWMIC("wmic cpu get processorid");
}
QString QUIHelper::getDiskNum()
{
    return getWMIC("wmic diskdrive where index=0 get serialnumber");
}
QString QUIHelper::getFileVersion(QString appName)
{
    QString vRetVersion;
...
bool QUIHelper::CheckAppRunningStatus(const QString &appName)
{
#ifdef Q_OS_WIN
...
void QUIHelper::KillAppProcess(const QString &appName)
{
#ifdef Q_OS_WIN
    QProcess p;
...
QString QUIHelper::generateUniqueRandomNumber()
{
    QString str;
    int i,j;
    ...
    for(i=0;i<6;i++)
    {
        str += QString("%1").arg(numbersList[i]);
    }
    str = "D"+str;
    return str;
}
...
{
    QString fileName = QString("%1/%2/%3_runinfo_%4_%5_%6.txt").arg(QUIHelper::appPath())
                       .arg(filePath).arg(QUIHelper::appName()).arg(QDate::currentDate().year()).arg(QDate::currentDate().month()).arg(QDate::currentDate().day());
...
{
    QString qssFile = ":/qss/black.css";


    if (style == QUIWidget::Style_Black) {
        qssFile = ":/qss/black.css";
    }
    QFile file(qssFile);
...
{
    QString qssRed = "QLabel{border:none;background-color:rgb(214,64,48);color:rgb(255,255,255);}QLabel::disabled{background:none;}";
    QString qssGreen = "QLabel{border:none;background-color:rgb(46,138,87);color:rgb(255,255,255);}QLabel::disabled{background:none;}";
    QString qssBlue = "QLabel{border:none;background-color:rgb(67,122,203);color:rgb(255,255,255);}QLabel::disabled{background:none;}";
    QString qssDark = "QLabel{border:none;background-color:rgb(75,75,75);color:rgb(255,255,255);}QLabel::disabled{background:none;}";
...
//CRC16-CCITT校验
quint16 QUIHelper::getModbus16ccitt(quint8 *data, int len)
{
    const quint16 table_16[256] = {
    ...
    return getRevCrc_16(data, len, 0xFFFF, table_16);
}
//CRC16校验
...
QString QUIHelper::byteArrayToHexStr(const QByteArray &data)
{
    ...
    //return temp.trimmed().toUpper();  //清除结尾的空格
    return temp.toUpper();
}
QList<QStringList> QUIHelper::getCSVdData(const QString &filter)
{
...
void QUIHelper::setCSVdData(const QString &filter, QList<QStringList> list)
{
   ...
   if(file.open(QIODevice::WriteOnly | QIODevice::Text)){
       ...
       {
         int colmax = list.at(i).size();
         for(int j = 0; j < colmax; j++)
         {
             QString string = list.at(i).at(j);
             out << string << ",";// 写入文件
...
void QUIHelper::setCSVdData(const QString &filter, QStringList list, int col)
{
    QList<QStringList> CAIdata = QUIHelper::getCSVdData(filter);
    QList<QStringList> data;
    for(int i =0 ; i<CAIdata.size(); i++){
        QStringList row;
        for(int j = 0; j<CAIdata.at(0).size(); j++)
        {
            if(j == col){
                if(i < list.size())
                ...
                row.append(CAIdata.at(i).at(j));

        }
         data.append(row);
    }

    int rowmax = data.size();
    QFile file(filter);
    if (file.size() == 0) {
        return;
    }
   if(file.open(QIODevice::WriteOnly | QIODevice::Text)){
       ...
       for(int i = 0; i < rowmax; i ++)
       {
         int colmax = data.at(i).size();
         for(int j = 0; j < colmax; j++)
         {
             QString string = data.at(i).at(j);
             out << string << ",";// 写入文件
...
QString QUIConfig::TextColor = "#080405";
QString QUIConfig::PanelColor = "#080405";
QString QUIConfig::BorderColor = "#080405";
QString QUIConfig::NormalColorStart = "#080405";
QString QUIConfig::NormalColorEnd = "#080405";
QString QUIConfig::DarkColorStart = "#080405";
QString QUIConfig::DarkColorEnd = "#080405";
QString QUIConfig::HighColor = "#080405";
